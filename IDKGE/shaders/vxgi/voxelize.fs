...
void main()
{
    ...
    vec4  albedo = texture(IDK_SSBO_materials[material_id][0], fsin_texcoords).rgba;
    // vec3  normal = texture(IDK_SSBO_materials[material_id][1], texcoords).xyz * 2.0 - 1.0;
    vec3  ao_r_m = texture(IDK_SSBO_materials[material_id][2], fsin_texcoords).rgb;
    float emissv = texture(IDK_SSBO_materials[material_id][3], fsin_texcoords).r;
